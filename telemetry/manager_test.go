...
func TestClientManager_GetOrCreateClient(t *testing.T) {
	...
	client1 := manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
	if client1 == nil {
	...
	client2 := manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
	if client2 != client1 {
...
func TestClientManager_GetOrCreateClient_DifferentHosts(t *testing.T) {
	...
	client1 := manager.getOrCreateClient(host1, "test-version", "test-ua", httpClient, cfg)
	client2 := manager.getOrCreateClient(host2, "test-version", "test-ua", httpClient, cfg)
...
func TestClientManager_ReleaseClient(t *testing.T) {
	...
	manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
	manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
...
func TestClientManager_ConcurrentAccessMultipleHosts(t *testing.T) {
	...
	for _, host := range hosts {
		for i := 0; i < numGoroutinesPerHost; i++ {
			...
			go func(h string) {
				defer wg.Done()
				_ = manager.getOrCreateClient(h, "test-version", "test-ua", httpClient, cfg)
			}(host)
		}
	}
	wg.Wait()
...
func TestClientManager_MultipleGetOrCreateSameClient(t *testing.T) {
	...
	client1 := manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
	client2 := manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
	client3 := manager.getOrCreateClient(host, "test-version", "test-ua", httpClient, cfg)
