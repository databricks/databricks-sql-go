...
func TestColumnsWithDirectResults(t *testing.T) {
	...
	d, err := NewRows(ctx, nil, client, nil, nil, nil, nil, nil)
	assert.Nil(t, err)
...
func TestRowsCloseOptimization(t *testing.T) {
	...
	rowSet, _ := NewRows(ctx, opHandle, client, nil, nil, nil, nil, nil)
	...
	closeCount = 0
	rowSet, _ = NewRows(ctx, opHandle, client, nil, directResults, nil, nil, nil)
	err = rowSet.Close()
	assert.Nil(t, err, "rows.Close should not throw an error")
	assert.Equal(t, 1, closeCount)
...
func TestGetArrowBatches(t *testing.T) {
	t.Run("with direct results", func(t *testing.T) {
		...
		rows, err := NewRows(ctx, nil, client, cfg, executeStatementResp.DirectResults, nil, nil, nil)
		assert.Nil(t, err)
	...
	t.Run("without direct results", func(t *testing.T) {
		...
		cfg := config.WithDefaults()
		rows, err := NewRows(ctx, nil, client, cfg, nil, nil, nil, nil)
		assert.Nil(t, err)
...
func TestFetchResultPage_PropagatesGetNextPageError(t *testing.T) {
	...
	executeStatementResp := cli_service.TExecuteStatementResp{}
	cfg := config.WithDefaults()
	rows, _ := NewRows(ctx, nil, client, cfg, executeStatementResp.DirectResults, nil, nil, nil)
