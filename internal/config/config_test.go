...
func TestParseConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "kernel backend param tokenCache=true sets TokenCacheEnabledDSN",
			args: args{dsn: "token:supersecret@example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a?tokenCache=true"},
			wantCfg: UserConfig{
				Protocol:             "https",
				Host:                 "example.cloud.databricks.com",
				Port:                 443,
				...
				RetryWaitMax:         30 * time.Second,
				CloudFetchConfig:     defCloudConfig,
				TokenCacheEnabledDSN: true,
			},
			wantURL: "https://example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a",
			wantErr: false,
		},
		{
			name: "kernel backend param tokenCache=false sets TokenCacheEnabledDSN to false",
			args: args{dsn: "token:supersecret@example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a?tokenCache=false"},
			wantCfg: UserConfig{
				Protocol:             "https",
				Host:                 "example.cloud.databricks.com",
				Port:                 443,
				...
				RetryWaitMax:         30 * time.Second,
				CloudFetchConfig:     defCloudConfig,
				TokenCacheEnabledDSN: false,
			},
			wantURL: "https://example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a",
			wantErr: false,
		},
		{
			name:    "malformed tokenCache is an error",
			args:    args{dsn: "token:supersecret@example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a?tokenCache=notabool"},
			wantCfg: UserConfig{},
			wantErr: true,
		},
	}
