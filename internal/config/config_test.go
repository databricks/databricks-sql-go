...
func TestParseConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "with telemetry_retry_count=0 (disable retries)",
			args: args{dsn: "token:supersecret@example.cloud.databricks.com:443/sql/1.0/endpoints/12346a5b5b0e123a?telemetry_retry_count=0"},
			wantCfg: UserConfig{
				Protocol:            "https",
				Host:                "example.cloud.databricks.com",
				Port:                443,
