...
func TestDbSqlErrors(t *testing.T) {
	...
	t.Run("NewExecutionErrorWithState carries the sqlstate", func(t *testing.T) {
		// The kernel backend has no Thrift TGetOperationStatusResp; it passes an
		// already-extracted sqlstate. The value must satisfy DBExecutionError and
		// surface that sqlstate — the parity property with NewExecutionError.
		cause := errors.New("cause")
		execError := NewExecutionErrorWithState(context.TODO(), "exec error", cause, "42P01")
		e := errors.Wrap(execError, "is wrapped")
